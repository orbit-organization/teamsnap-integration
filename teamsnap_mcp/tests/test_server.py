...
class TestToolOutput:
    ...
    async def test_list_teams_output_format(self, mock_env, sample_search_results):
        ...
        with patch.object(
            TeamSnapAsyncClient, "search_teams", new_callable=AsyncMock
        ) as mock_search:
    ...
    async def test_list_events_output_format(self, mock_env, sample_search_results):
        ...
        with patch.object(
            TeamSnapAsyncClient, "search_events", new_callable=AsyncMock
        ) as mock_search:
            mock_search.return_value = sample_search_results["collection"]["items"]
    ...
    async def test_create_event_success_format(self, mock_env, sample_event_data):
        ...
        with patch("os.getenv") as mock_getenv:
            ...
            with patch.object(
                TeamSnapAsyncClient, "create_event", new_callable=AsyncMock
            ) as mock_create:
                mock_create.return_value = sample_event_data

                result = await create_event(
                    team_id=12345, name="Practice", start_date="2025-01-15T14:00:00Z"
                )
                ...
                assert result[0].type == "text"
                assert (
                    "Successfully created" in result[0].text or "✅" in result[0].text
                )
    ...
    async def test_update_availability_valid_status(self, mock_env):
        ...
        with patch.object(
            TeamSnapAsyncClient, "update_availability", new_callable=AsyncMock
        ) as mock_update:
