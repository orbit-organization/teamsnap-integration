...
async def list_events(team_id: int) -> list[TextContent]:
    ...
    async with get_client() as client:
        ...
        if not events:
            return [
                TextContent(type="text", text=f"No events found for team {team_id}.")
            ]
...
async def list_members(team_id: int) -> list[TextContent]:
    ...
    async with get_client() as client:
        ...
        if not members:
            return [
                TextContent(type="text", text=f"No members found for team {team_id}.")
            ]
...
async def get_event_availability(event_id: int) -> list[TextContent]:
    ...
    async with get_client() as client:
        ...
        if not availabilities:
            return [
                TextContent(
                    type="text", text=f"No availability responses for event {event_id}."
                )
            ]
...
async def list_assignments(event_id: int) -> list[TextContent]:
    ...
    async with get_client() as client:
        ...
        if not assignments:
            return [
                TextContent(
                    type="text", text=f"No assignments found for event {event_id}."
                )
            ]
...
async def list_locations(team_id: int) -> list[TextContent]:
    ...
    async with get_client() as client:
        ...
        if not locations:
            return [
                TextContent(type="text", text=f"No locations found for team {team_id}.")
            ]
        ...
        try:
            ...
            if items:
                ...
                return [
                    TextContent(
                        type="text",
                        text="Event created but unable to retrieve details.",
                    )
                ]
...
async def update_event(
    ...
    async with get_client() as client:
        try:
            ...
            await client.update_event(event_id, **fields)
...
async def delete_event(event_id: int) -> list[TextContent]:
    ...
    async with get_client() as client:
        try:
            ...
            return [
                TextContent(
                    type="text", text=f"✅ Successfully deleted event {event_id}"
                )
            ]
        ...
        try:
            ...
            if items:
                ...
                return [
                    TextContent(
                        type="text",
                        text="Member created but unable to retrieve details.",
                    )
                ]

        except Exception as e:
            return [
                TextContent(type="text", text=f"❌ Error creating member: {str(e)}")
            ]
...
async def update_member(
    ...
    async with get_client() as client:
        try:
            ...
            await client.update_member(member_id, **fields)
            ...
            return [
                TextContent(type="text", text=f"❌ Error updating member: {str(e)}")
            ]
...
async def delete_member(member_id: int) -> list[TextContent]:
    ...
    async with get_client() as client:
        try:
            ...
            return [
                TextContent(
                    type="text", text=f"✅ Successfully removed member {member_id}"
                )
            ]

        except Exception as e:
            return [
                TextContent(type="text", text=f"❌ Error deleting member: {str(e)}")
            ]


@mcp.tool()
async def update_availability(availability_id: int, status: str) -> list[TextContent]:
    """
    ...
    async with get_client() as client:
        try:
            ...
            await client.update_availability(availability_id, status.lower())
            ...
            return [
                TextContent(
                    type="text", text=f"❌ Error updating availability: {str(e)}"
                )
            ]
...
async def create_assignment(
    event_id: int, member_id: int, description: str
) -> list[TextContent]:
    """
    ...
    async with get_client() as client:
        try:
            response = await client.create_assignment(
                event_id=event_id, member_id=member_id, description=description
            )
            ...
            if items:
                ...
                return [
                    TextContent(
                        type="text",
                        text="Assignment created but unable to retrieve details.",
                    )
                ]

        except Exception as e:
            return [
                TextContent(type="text", text=f"❌ Error creating assignment: {str(e)}")
            ]
    ...
    async with get_client() as client:
        try:
            ...
            return [
                TextContent(
                    type="text",
                    text=f"✅ Successfully deleted assignment {assignment_id}",
                )
            ]

        except Exception as e:
            return [
                TextContent(type="text", text=f"❌ Error deleting assignment: {str(e)}")
            ]


@mcp.tool()
async def create_location(
    team_id: int, name: str, address: Optional[str] = None
) -> list[TextContent]:
    """
    ...
    async with get_client() as client:
        try:
            response = await client.create_location(
                team_id=team_id, name=name, address=address
            )
            ...
            if items:
                ...
                return [
                    TextContent(
                        type="text",
                        text="Location created but unable to retrieve details.",
                    )
                ]

        except Exception as e:
            return [
                TextContent(type="text", text=f"❌ Error creating location: {str(e)}")
            ]
...
async def delete_location(location_id: int) -> list[TextContent]:
    ...
    async with get_client() as client:
        try:
            ...
            return [
                TextContent(
                    type="text", text=f"✅ Successfully deleted location {location_id}"
                )
            ]

        except Exception as e:
            return [
                TextContent(type="text", text=f"❌ Error deleting location: {str(e)}")
            ]
