...
def get_current_api_state(client):
    ...
    endpoints = {
        'links': [{'rel': link.get('rel'), 'href': link.get('href'), 'deprecated': link.get('deprecated', False)}
                  for link in links],
        'queries': [{'rel': query.get('rel'), 'href': query.get('href')}
                    for query in queries],
        'commands': [{'rel': cmd.get('rel'), 'href': cmd.get('href'), 'method': cmd.get('method')}
                     for cmd in commands]
    }
...
def compare_states(old_state, new_state):
    ...
    changes = {
        ...
        'removed_endpoints': [],
        'new_deprecations': [],
        'deprecated_count_changed': old_state['deprecated_count'] != new_state['deprecated_count']
    }
...
def main():
    ...
    parser.add_argument('--save', action='store_true', help='Save current API state as snapshot')
    parser.add_argument('--compare', action='store_true', help='Compare with previous snapshot')
    parser.add_argument('--show-deprecations', action='store_true', help='Show all deprecated endpoints')
